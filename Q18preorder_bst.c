...
3. **In the main function**:  
   ...
   - Perform Preorder Traversal using the traversal function.  
...
6. **Define Preorder Traversal Function**:  
   - If the current node is not NULL:  
     - Print the node's value.  
     - Recursively traverse the left subtree.  
     - Recursively traverse the right subtree.  
7. **Call the traversal function from `main()`** to display the BST elements in preorder.  
8. **End the program**.  
